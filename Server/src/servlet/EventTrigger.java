...
@WebServlet("/EventTrigger")
public class EventTrigger extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		String keyword = getCookieValue(cookies, "keyword");
		if (keyword != null) {
			response.setContentType("text/event-stream");
			response.setCharacterEncoding("UTF-8");
			long scoreTimeStamp = Long.valueOf(getCookieValue(cookies,
					"scoreTimeStamp"));
			JDBC dao;
			try {
				dao = JDBC.getInstance();
				String serverTweetsJson = dao.getLatestScoredTweets(keyword,
						scoreTimeStamp);
				response.getWriter().write("data: " + serverTweetsJson + "\n\n");
